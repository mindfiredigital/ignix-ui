...
| `columns` | `Array<{ key: string; label: string; sortable?: boolean; align?: 'left' \| 'right' \| 'center' }>` | **required** | Column definitions. Use `sortable: false` to disable sorting, and `align` to set alignment ('left' \| 'right' \| 'center'). |
