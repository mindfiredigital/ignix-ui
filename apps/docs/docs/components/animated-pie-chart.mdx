...
description: An animated SVG pie and donut chart with spring-animated slices, hover effects, percentage labels, interactive legend, donut center labels, and four theme variants.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import AnimatedPieChartDemo from '@site/src/components/Demo/AnimatedPieChartDemo';

The `AnimatedPieChart` renders animated SVG pie and donut charts. Each slice springs in individually on mount, scales up on hover, and pairs with an interactive legend for slice highlighting. Donut mode supports configurable inner radius and custom center labels.

<AnimatedPieChartDemo />
...
<Tabs>
  <TabItem value="cli" label="CLI" default>
    ```bash
    ignix add component animated-pie-chart
    ```
  </TabItem>
...
| :--- | :--- | :--- | :--- |
| `data` | `PieDataPoint[]` | **required** | Array of slice data with label, value, and optional color. |
| `title` | `string` | - | Optional chart title. |
| `subtitle` | `string` | - | Optional subtitle below the title. |
| `radius` | `number` | `110` | Outer radius of the pie/donut in pixels. |
| `donutRatio` | `number` | `0` | Inner radius ratio `(0–1)`. Set `> 0` to enable donut mode. |
| `centerLabel` | `string` | - | Primary label displayed in the donut center (donut mode only). |
| `centerSubLabel` | `string` | - | Secondary label below `centerLabel`. |
| `showLegend` | `boolean` | `true` | Show the interactive side legend. |
| `showPercentages` | `boolean` | `false` | Render percentage annotations on each slice. |
| `animate` | `boolean` | `true` | Enable spring entrance animations. |
...
| :--- | :--- | :--- |
| `label` | `string` | Slice label displayed in the legend and tooltip. |
| `value` | `number` | Numeric value used to compute the slice angle. |
| `color` | `string` | Optional CSS color. Falls back to the built-in palette. |
