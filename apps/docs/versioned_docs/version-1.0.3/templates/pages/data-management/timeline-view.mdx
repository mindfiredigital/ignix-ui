...
sidebar_position: 3
title: Timeline View
description: A filterable, sortable timeline that renders vertically or horizontally with four visual variants, three status states, click-to-open detail drawer, and a shimmer loading state.
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import TimelineViewPageDemo from '@site/src/components/Demo/TimelineViewPageDemo';

A flexible timeline component for milestones, release histories, audit logs, project plans, and onboarding flows. Ships with four visual variants (`default`, `minimal`, `compact`, `glow`), responsive vertical/horizontal layouts, status-based color coding, filter pills with live counts, a shimmer skeleton loading state, and a built-in click-to-open detail drawer with custom render-prop support.

<TimelineViewPageDemo />
...
const NODE_RING: Record<TimelineStatus, string> = {
    completed: "bg-success border-success ring-4 ring-success/25",
    in_progress: "bg-warning border-warning animate-pulse ring-4 ring-warning/25",
...
export interface TimelineProps {
    ...
    /** Parent-controlled click handler. Suppresses built-in drawer when provided. */
    onItemClick?: (item: TimelineItem) => void;
    /** Render prop for custom drawer body. */
...
export function Timeline({
    ...
    const showVertical = orientation === "vertical" || orientation === "auto";
    const showHorizontal = orientation === "horizontal" || orientation === "auto";
        ...
        <div className="space-y-8">
            ...
            {visible.length === 0 ? (
                ...
                <>
                    {showVertical && (
                        <div className={cn("block", orientation === "auto" && "md:hidden")}>
                            <VerticalTimeline items={visible} variant={variant} onItemClick={handleItemClick} />
                        </div>
                    )}
                    {showHorizontal && (
                        <div className={cn(orientation === "auto" ? "hidden md:block" : "block")}>
                            <HorizontalTimeline items={visible} variant={variant} onItemClick={handleItemClick} />
                        </div>
                    )}
                </>
            )}
...
function DefaultTimelineDetails({ item }: { item: TimelineItem }) {
    return (
        <div className="-mx-4 -mt-4">
            <div
                aria-hidden
                className={cn("h-1 w-full bg-gradient-to-r", STATUS_ACCENT[item.status])}
            />
...
|------|------|---------|-------------|
| `items` | `TimelineItem[]` | `Required` | Items rendered on the timeline. Auto-sorted ascending by `date`. |
...
| `showFilters` | `boolean` | `true` | Show or hide the filter pill row. |
| `isLoading` | `boolean` | `false` | Show shimmer skeleton state. |
| `skeletonCount` | `number` | `3` | Number of skeleton cards rendered when `isLoading`. |
| `enableDetails` | `boolean` | `true` | Enable built-in click-to-open detail drawer. Set `false` for static cards. |
| `onItemClick` | `(item: TimelineItem) => void` | `-` | Parent-controlled click handler. Suppresses built-in drawer when provided. |
...
|-------|------|-------------|
| `id` | `string` | Unique item id. Used for React keys. |
| `title` | `string` | Headline shown in the card and drawer header. |
| `description` | `string` | Optional secondary line below the title. |
| `date` | `string` | ISO date string. Drives sorting and the formatted date label. |
| `status` | `"completed" \| "in_progress" \| "pending"` | Status used for color coding, the node ring, and the status badge. |
| `meta` | `string` | Optional small uppercase label rendered above the title (e.g. `"Milestone 01"`). |

### TimelineStatus

| Value | Visual | Description |
|-------|--------|-------------|
| `completed` | Success / green | Completed item. Filled node with success ring. |
| `in_progress` | Warning / amber | Active item. Pulsing node with warning ring. |
| `pending` | Neutral / muted | Future item. Hollow node with muted ring. |

### TimelineVariant

| Value | Description |
|-------|-------------|
| `default` | Status-tinted card with colored left border and badge. |
| `minimal` | Borderless, low-chrome layout. Best for dense timelines. |
| `compact` | Single-line card with title, date, and status only. |
| `glow` | Glassmorphic card with gradient and ambient color blobs matching status. |
