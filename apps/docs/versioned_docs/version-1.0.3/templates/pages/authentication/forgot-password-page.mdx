...
Here's a simple example showing how to use the `ForgotPasswordPage` component in your application:
...
```tsx
import { ForgotPasswordPage } from '@site/src/components/UI/forgot-password';
...
|------|------|---------|-------------|
| `forgotPasswordHeader` | `{ head: string, para: string }` | - | Header configuration with title and description |
| `formCardHeader` | `React.ReactNode` | - | Custom header React node (alternative to `forgotPasswordHeader`) |
| `loginLink` | `React.ReactNode` | - | Custom login link component |
| `button` | `React.ReactNode` | - | Custom submit button component |
| `input` | `React.ReactNode` | - | Custom input component |
| `variant` | `"default" \| "dark" \| "gradientAurora" \| "gradientEmeraldWave" \| "gradientRoyal" \| "gradientOceanNight"` | `"default"` | Visual theme variant for the page |
| `inputVariant` | `"clean" \| "underline" \| "floating" \| "borderGlow" \| "shimmer" \| "particles" \| "slide" \| "scale" \| "rotate" \| "bounce" \| "elastic" \| "glow" \| "shake" \| "wave" \| "typewriter" \| "magnetic" \| "pulse" \| "borderBeam" \| "ripple" \| "particleField" \| "tilt3D"` | `"clean"` | Animation variant for the input field |
| `animationVariant` | `"bounce" \| "bounceSlow" \| "bounceFast" \| "bounceSmooth" \| "bounceJelly" \| "rotateClockwiseSlow" \| "rotateClockwiseFast" \| "rotateAntiClockwiseSlow" \| "rotateAntiClockwiseFast" \| "rotatePingPong" \| "scaleUp" \| "scaleDown" \| "scaleHeartBeat" \| "flipX" \| "flipY" \| "flipCard" \| "fadeBlink" \| "fadeInOut" \| "press3D" \| "press3DSoft" \| "press3DHard" \| "press3DPop" \| "press3DDepth" \| "spinSlow"` | `"press3D"` | Animation variant for the submit button |
| `headerAnimation` | `"fadeUp" \| "scaleIn" \| "slideLeft" \| "slideRight" \| "flipIn"` | `"fadeUp"` | Animation variant for the header |
| `status` | `"idle" \| "loading" \| "success" \| "error"` | `"idle"` | Current status of the form |
| `errorMessage` | `string` | - | Custom error message to display |
| `successMessage` | `string` | `"Check your email for a reset link"` | Success message displayed after submission |
...
| `onSubmit` | `(email: string) => void \| Promise<void>` | - | Callback function called when form is submitted with valid email |
| `onNavigateTo` | `() => void` | - | Callback function called when navigate/back button is clicked |
| `className` | `string` | - | Additional CSS classes to apply to the component |
| `captcha` | `React.ReactNode` | - | CAPTCHA component to display |
| `captchaVerified` | `boolean` | - | Whether the CAPTCHA is verified (required if `captcha` is provided) |
