...
import { PricingGridBasicDemo, PricingGridModernDemo } from '@site/src/components/Demo/PricingGridDemo';
...
|-----|------|---------|-------------|
| `title` | `string` | `"Our Pricing Table"` | Heading text displayed above the pricing grid. |
| `description` | `string` | `"Choose the plan that fits your needs and plans"` | Supporting description text below the title. |
| `plans` | `PlanProps[]` | — | List of pricing plans to render. |
| `variant` | `"default" \| "dark"` | `"dark"` | Visual theme variant for the grid and cards. |
| `modernUI` | `"basic" \| "vector" \| "advance"` | `"basic"` | Controls the overall card layout and visual style. |
| `pricingBadgePosition` | `"none" \| "top" \| "middle"` | `"middle"` | Position of the price badge inside each card. |
| `allowDifferentCardColors` | `boolean` | `false` | Allows each card to have a different background/color style. |
| `animation` | `"none" \| "fadeIn" \| "slideUp" \| "scaleIn" \| "flipIn" \| "bounceIn" \| "floatIn"` | `"slideUp"` | Entry animation applied to pricing cards. |
| `interactive` | `"none" \| "hover" \| "press" \| "lift" \| "tilt" \| "glow"` | `"press"` | Hover/interaction effect for cards. |
| `table` | `boolean` | `false` | Displays features in a comparison table instead of a list. |
| `className` | `string` | — | Custom class names for the outer grid container. |
...
| Prop | Type | Default | Description |
|-----|------|---------|-------------|
| `id` | `string` | — | Optional unique identifier for the plan. |
...
| `icon` | `React.ElementType` | — | Optional icon shown in modern layouts. |
| `highlighted` | `boolean` | `false` | Visually emphasizes the plan as recommended. |
| `gradient` | `string` | — | Optional gradient background for the plan card. |
| `buttonColor` | `string` | — | Custom button or card color class. |
...
| `available` | `boolean` | `true` | Indicates whether the feature is available. |
| `icon` | `React.ElementType` | — | Optional icon to override the default check/cross icon. |
