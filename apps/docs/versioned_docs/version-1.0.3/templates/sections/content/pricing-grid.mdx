...
| `description` | `string` | `"Choose the plan that fits your needs"` | Supporting description text below the title. |
...
| `allowDifferentCardColors` | `boolean` | `false` | Enables per-plan gradients/colors (applies mainly to `vector` UI). |
...
| `table` | `boolean` | `false` | Displays features as a table instead of a list. |
...
| `value` | `string` | — | Controlled value for the pricing toggle. |
| `onValueChange` | `(value: string) => void` | — | Callback fired when the toggle value changes. |
| `className` | `string` | — | Custom class names for the outer grid container. |
| `currentPlan` | `number` | `1` | Marks the plan as the currently active plan. |
...
| `buttonColor` | `string` | — | Optional button or card color. |
...
| `icon` | `React.ElementType` | — | Optional icon to override the default check/cross icon (`Check`/`X`). |
...
|------|------|-------------|
| `label` | `React.ReactNode` | Label displayed next to the toggle option. |
| `value` | `string` | Value associated with the toggle option. |
