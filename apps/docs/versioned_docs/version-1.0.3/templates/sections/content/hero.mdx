...
import { HeroDemo, HeroSplitDemo, HeroVideoDemo } from '@site/src/components/Demo/HeroDemo';
...
export const Hero: React.FC<HeroProps> = ({
  ...
  React.Children.forEach(children, (child) => {
    if (isHeroImage(child)) {
      // Clone the child and pass video control props
      const clonedChild = React.cloneElement(child, {
        videoRef,
        isPlaying,
        onPlayPause: handlePlayPause,
      });
      backgroundImages.push(clonedChild);
...
### Hero with Features, Badge, Stats
...
| `split` | `boolean` | - | Enable side-by-side layout when HeroImage with left/right position is present |
...
| Prop | Type | Default | Description |
|------|------|---------|-------------|
| `src` | `string` | **required** | Video source URL (supports .mp4, .webm, .ogg, .mov, or .gif) |
| `overlayOpacity` | `number` | `50` | Overlay opacity (0-100). Minimum enforced at 40 for text readability. Values are rounded to nearest 10 |
| `showPlayPause` | `boolean` | `false` | Show play/pause control button |
| `fallbackImage` | `string` | - | Fallback image URL shown when video fails to load or is not supported |
| `className` | `string` | - | Custom className |

**Note:** HeroVideo automatically detects GIF files and renders them as animated images. For video files, it supports MP4, WebM, OGG, and MOV formats. The component includes automatic fallback handling for unsupported formats.
...
| `alt` | `string` | `'background Image'` | Image alt text |
| `position` | `'left' \| 'right' \| 'center' \| 'background'` | `'background'` | Image position |
| `overlayOpacity` | `number` | `40` | Overlay opacity (0-100) for background images. Values are rounded to nearest 10 (0, 10, 20, ..., 100) |
| `className` | `string` | - | Custom className |
