...
2. **Composable (nested)** — Compose `ModalOverlay`, `ModalContent`, `ModalHeader`, `ModalBody`, and `ModalFooter` yourself (similar to Accordion). Use `Modal.colorSchemeConfig` to apply the same color schemes.
